...
| Include `error.message` and the rejected instance in structured-schema diagnostics. | The real corrupt-row test exposes the provenance marker and exceeds its diagnostic bound. | `test_recall_records_contains_corrupt_provenance_at_served_boundary`: failed with the raw marker present. | Schema failures identify the path and validator without reflecting attacker-controlled values. |
| Disable the raw-string precheck before `_callToolResult`. | The wrapper spy receives the 2 MiB dispatch error. | `test_recall_records_served_boundary_replaces_oversized_dispatch_error`: failed with three wrapper calls and one giant argument. | Oversized raw errors are rejected before envelope serialization. |
| Bypass the final serialized-envelope size comparison. | A raw string below the cap but above it after JSON escaping is served. | `test_recall_records_served_boundary_counts_escaping_overhead`: failed with a 1,200,129-byte envelope. | The served `model_dump_json()` envelope, not only raw UTF-8 input, is capped. |
| Apply the final guard to legacy `pensive_recall`. | The historical oversized error is replaced. | `test_legacy_served_boundary_preserves_oversized_dispatch_error`: failed with the 52-character bounded replacement. | The new containment policy is scoped to `recall_records`. |
| Return the bounded replacement even when the configured cap cannot hold it. | The server reports the ordinary oversize message instead of the configuration failure. | `test_recall_records_served_boundary_fails_when_cap_cannot_hold_error`: failed with the wrong 52-character error. | An impossible cap fails loudly through the MCP error boundary. |
...
For the served-error follow-up, one load-bearing assertion in each of the five new boundary tests was inverted in one temporary edit. The focused run produced `5 failed, 47 deselected`. After restoration, the same selection produced `5 passed, 47 deselected`. The inversions covered concise corrupt-row diagnostics, oversized raw-error replacement, escaping-aware envelope measurement, impossible-cap failure, and legacy byte preservation.
